...
namespace Opal_Job\Libraries\Money;
...
class Money
{
     ...
     * Currency instance
     *
     * @var \Opal_Job\Libraries\Money\Currency
     */
    protected $currency;
    ...
    public function __construct( $currency = 'USD' ) {
        ...
        if ( !isset( self::$instances[$currency] ) ) { 
            self::$instances[$currency] = (is_string($currency) ? new Currency($currency) : $currency);
        } 
        
        $this->currency = self::$instances[$currency];
    }
