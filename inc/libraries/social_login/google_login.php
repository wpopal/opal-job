...
class Google_Login {
	...
	public function __construct() {
		...
		$this->google_client_secret = opaljob_get_option( 'google_client_secret', '' );
		$this->google_api_key       = opaljob_get_option( 'google_api_key', '' );
		...
		add_action( 'wp_ajax_opaljob_ajax_redirect_google_login_link', [ $this, 'ajax_redirect_google_login_link' ] );
		add_action( 'wp_ajax_nopriv_opaljob_ajax_redirect_google_login_link', [ $this, 'ajax_redirect_google_login_link' ] );
	}
	...
	public function ajax_redirect_google_login_link() {
		if ( 'off' === opaljob_get_option( 'enable_google_login' ) ) {
			...
			try {
				...
				if ( ! is_user_logged_in() ) {
					...
					if ( $ID ) {
						 ...
						 */
						do_action( 'opaljob_after_create_google_user_new_email', $user_id );
	...
	public function redirect() {
		$redirect = Social_Login::get_redirect_url();
	...
	public function signon( $credentials ) {
		...
		if ( is_wp_error( $user_signon ) ) {
			 ...
			 */
			do_action( 'opaljob_after_signon_successfully', $credentials );

			$redirect = opaljob_get_user_management_page_uri();
