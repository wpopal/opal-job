...
( function ( window, document, $, opal, undefined ) {
    ...
    opal.init = function () {
        ...
        // Init tab form.
        opal.initTabForm( $( '.opaljob-form-data-tabs' ) );

        // Make File List drag/drop sortable:
        opal.makeListSortable();

        // Init time/date/color pickers
        opal.initPickers( $metabox.find( 'input[type="text"].opaljob-colorpicker' ),
            $metabox.find( 'input[type="text"].opaljob-datepicker' ) );

        $metabox
            .on( 'change', '.opaljob_upload_file', function () {
                opal.media.field = $( this ).attr( 'id' );
                $id( opal.media.field + '_id' ).val( '' );
            } )
            // Media/file management
            .on( 'click', '.opaljob-upload-button', opal.handleMedia )
            .on( 'click', '.opaljob-attach-list li, .opaljob-media-status .img-status img, .opaljob-media-status' +
                ' .file-status >' +
                ' span', opal.handleFileClick )
            .on( 'click', '.opaljob-remove-file-button', opal.handleRemoveMedia );
    ...
    opal.initTabForm = function ( $tabs ) {
        $( $tabs ).each( function () {
            ...
            $( 'a', $tab ).click( function () {
                $( '.opaljob_options_panel', $parent ).hide();
    ...
    opal.makeListSortable = function() {
        var $filelist = opal.metabox().find( '.opaljob-media-status.opaljob-attach-list' );
        if ( $filelist.length ) {
    ...
    opal.datePickerSetupOpts = function ( fieldOpts, options, method ) {
        ...
        options.beforeShow = function ( input, inst ) {
            ...
            $id( 'ui-datepicker-div' ).addClass( 'opaljob-element' );
        ...
        options.onClose = function ( dateText, inst ) {
            ...
            var $picker = $id( 'ui-datepicker-div' ).removeClass( 'opaljob-element' ).hide();
    ...
    opal.handleMedia = function ( evt ) {
        ...
        var $el = $( this );
        opal.attach_id = !$el.hasClass( 'opaljob-upload-list' ) ? $el.closest( '.opaljob-field-wrap' )
                                                                     .find( '.opaljob-upload-file-id' )
                                                                     .val() : false;
        // Clean up default 0 value
        opal.attach_id = '0' !== opal.attach_id ? opal.attach_id : false;

        opal._handleMedia( $el.prev( 'input.opaljob-upload-file' ).attr( 'id' ),
            $el.hasClass( 'opaljob-upload-list' ) );
    };
    ...
    opal.handleFileClick = function ( evt ) {
        ...
        evt.preventDefault();
        var $el = $( this );
        var $td = $el.closest( '.opaljob-field-wrap' );
        var isList = $td.find( '.opaljob-upload-button' ).hasClass( 'opaljob-upload-list' );
        opal.attach_id = isList ? $el.find( 'input[type="hidden"]' ).data( 'id' ) : $td.find(
            '.opaljob-upload-file-id' ).val();

        if ( opal.attach_id ) {
            opal._handleMedia( $td.find( 'input.opaljob-upload-file' ).attr( 'id' ), isList, opal.attach_id );
        }
    };
        ...
        handlers.list = function ( selection, returnIt ) {
            ...
            if ( !handlers.list.templates ) {
                handlers.list.templates = {
                    image: wp.template( 'opaljob-list-image' ),
                    file: wp.template( 'opaljob-list-file' ),
                };
            }
            ...
            if ( !returnIt ) {
                // Append each item from our fileGroup array to .opaljob-media-status
                media.$field.siblings( '.opaljob-media-status' ).append( fileGroup );
            } else {
                return fileGroup;
            }
        ...
        handlers.single = function ( selection ) {
            if ( !handlers.single.templates ) {
                handlers.single.templates = {
                    image: wp.template( 'opaljob-single-image' ),
                    file: wp.template( 'opaljob-single-file' ),
                };
            }
            ...
            media.$field.siblings( '.opaljob-media-status' ).slideDown().html( attachmentHtml );
        };
    ...
    opal.handleRemoveMedia = function ( evt ) {
        evt.preventDefault();
        var $this = $( this );
        if ( $this.is( '.opaljob-attach-list .opaljob-remove-file-button' ) ) {
            $this.parents( '.opaljob-media-item' ).remove();
            return false;
        }
        ...
        $this.parents( '.opaljob-media-status' ).html( '' );
    ...
    opal.groupFields = function ( $group_containers ) {
        ...
        var changeIndexes = function () {
            $( ".opal-row-repeater", $group_containers ).each( function( i ) {
        ...
        $group_containers.each( function() {
            var $_this = $( this ); 
            var $template = $( ".opaljob-template", this ); 
            ...
            $(  $_this ).on( 'click', ".opal-row-repeater button" , function() {
             ...
             $(  $_this ).on( 'click', ".opal-row-repeater .opaljob-remove" , function() {
                var group = $( this ).parent().parent().parent().parent();
                $( group ).remove();
                changeIndexes();
            } );
    ...
    opal.metabox = function () {
        ...
        opal.$metabox = $( '.js-opaljob-metabox-wrap' );
        return opal.$metabox;
    };
