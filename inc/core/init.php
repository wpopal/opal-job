	...
	private function define_admin_metabox() {
		...
		foreach( $metaboxes as $key => $metabox_class ) {
			...
			if( $metabox->get_mode() == 'taxonomy' ) {
				...
				add_action( 'created_term', array( $metabox, 'save_term' ), 10, 3 );
				add_action( 'edited_terms', array( $metabox, 'save_term' ), 10, 2 );
				add_action( 'delete_term', array( $metabox, 'delete' ), 10, 3 );

			} else if( $metabox->get_mode() == 'user' ) { 

				add_action( 'show_user_profile', array( $metabox, 'output' ), 10 );
				add_action( 'edit_user_profile', array( $metabox, 'output' ), 10 );

				add_action( 'personal_options_update', array( $metabox, 'save' ) );
				add_action( 'edit_user_profile_update', array( $metabox, 'save' ) );
