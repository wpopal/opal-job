...
class Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'opaljob_pre_sanitize', $sanitized );
		...
		switch ( $this->type ) {
			...
			default:
				if ( is_array( $this->data ) ) {
					$sanitized = array_map( 'opaljob_clean', $this->data );
				}

				$sanitized = is_scalar( $this->data ) ? sanitize_text_field( wp_unslash( $this->data ) ) : $this->data;
		} // switch

		/**
		 * Add additional santization after the default .
		 */
		do_action( 'opaljob_post_sanitize', $sanitized );
